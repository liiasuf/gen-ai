...
Домашнее задание 5: добавлен JSONL-лог шагов в trace.jsonl (задание 2).
...
# Набор инструментов — добавлен compare_periods (задание 1)
TOOLS_IMPL = {
...
# Путь к файлу с трассировками
TRACE_PATH = Path(__file__).resolve().parent / "trace.jsonl"

# ===========================================================================
# JSONL-трассировка (задание 2)
# ===========================================================================

def _write_trace(run_id: str, record: dict) -> None:
    """Дописать одну запись в trace.jsonl (режим 'a')."""
...
SUBMIT_SCHEMA = {
    ...
    "function": {
        "name": "submit_answer",
        "description": (
            "Вызови ТОЛЬКО когда данных достаточно для финального ответа. "
...
# ===========================================================================
# Блок 9: кэш детерминированных инструментов
# ===========================================================================
...
- compare_periods:  сравнить значение метрики в двух периодах (delta, ratio)
                    Используй его, если вопрос «во сколько раз», «насколько вырос»,
                    «сравни X в периоде A и периоде B».

Алгоритм:
1. Разложи вопрос: какие числа нужны и в каком порядке.
2. Если нужно сравнение двух дат — используй compare_periods (один вызов вместо двух).
3. Арифметику считай ТОЛЬКО через calculate.
...
def _exec_one(tc, cache: Optional[dict] = None) -> tuple[Any, dict, dict]:
    """Исполнить один вызов инструмента. Любую ошибку → obs={'error': ...}."""
    ...
    return ic.chat.completions.create(
        ...
        messages=[
            {"role": "system", "content": CRITIC_SYSTEM},
            {
                "role": "user",
                "content": (
                    f"Ответ агента: «{answer.answer}» (value={answer.value} {answer.unit}).\n"
                    f"Лог инструментов:\n{facts or '(пусто)'}"
                ),
            },
        ],
...
def _finish(
    ...
    *,
    ...
    if track_cost and usage_log:
        ...
        print(f" Итого: {total_in} вход + {total_out} выход токенов, ~${total_cost:.5f}.")

    if use_cache and verbose:
        print(f" [кэш] попаданий {CACHE_STATS['hits']}, промахов {CACHE_STATS['misses']}")
...
def run_agent(
    ...
    *,
    ...
    """ReAct-цикл с JSONL-логом шагов (задание 2)."""
    ...
    # Уникальный идентификатор прогона (задание 2)
    ...
    for step in range(1, max_iter + 1):
        ...
        if not msg.tool_calls:
            ...
            # Задание 2: записываем финал в trace.jsonl
            _write_trace(run_id, {"step": step, "final": msg.content})

            return _finish(
                {"answer": msg.content, "structured": None, "trace": trace, "steps": step},
                usage_log,
                track_cost=track_cost, use_cache=use_cache, verbose=verbose,
            )
        ...
        if others:
            ...
            for tc, args, obs in results:
                ...
                # Задание 2: пишем каждый вызов инструмента в JSONL
                _write_trace(run_id, entry)

                if verbose:
                    print(
                        f"  {tc.function.name}({args}) -> "
                        f"{json.dumps(obs, ensure_ascii=False)[:140]}"
                    )

        # ── Финал через submit_answer (блок 7) ────────────────────────────
            ...
            if use_critic:
                ...
                if not verdict.ok:
                    messages.append({
                        ...
                        "tool_call_id": submit.id,
                        "content": (
                            f"Ревизор отклонил: {verdict.issue}. "
                            "Перепроверь и вызови submit_answer заново."
                        ),
                    })
                    continue

            messages.append({"role": "tool", "tool_call_id": submit.id, "content": "ответ принят"})

            # Задание 2: финал через submit_answer тоже в лог
            _write_trace(run_id, {"step": step, "final": ans.answer})

            return _finish(
                {"answer": ans.answer, "structured": ans, "trace": trace, "steps": step},
                usage_log,
                track_cost=track_cost, use_cache=use_cache, verbose=verbose,
            )

    # Лимит шагов исчерпан
    _write_trace(run_id, {"step": max_iter, "final": None, "error": f"max_iter={max_iter} исчерпан"})

    return _finish(
        {
            "answer": None, "structured": None, "trace": trace, "steps": max_iter,
...
def main():
    ...
    ap.add_argument("--parallel", action="store_true",
                    help="Параллельные вызовы инструментов")
    ap.add_argument("--structured", action="store_true",
                    help="Структурный финал через submit_answer")
    ap.add_argument("--critic",   action="store_true",
                    help="Самопроверка финала (нужен --structured)")
    ap.add_argument("--cache",    action="store_true",
                    help="Кэш детерминированных инструментов")
    ap.add_argument("--cost",     action="store_true",
                    help="Показать токены и стоимость по шагам")
    ap.add_argument("--trace",    type=Path, default=None,
                    help="Куда сохранить JSON-лог (доп., помимо trace.jsonl)")
    ...
    if s:
        print(s.answer)
        print(
            f"value={s.value} {s.unit or ''} | "
            f"sources={s.sources} | confidence={s.confidence:.2f}"
        )
    else:
