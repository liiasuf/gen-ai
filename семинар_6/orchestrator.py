...
Домашнее задание 6:
- Задание 1: validate_plan() — схема-валидатор между Планировщиком и Исполнителем
- Задание 2: _topological_levels() + execute_level() с ThreadPoolExecutor
- Задание 3: critic() вызывается с temperature=0.7 (задаётся в critic.py)

Запуск:
    python orchestrator.py "Во сколько раз USD вырос с 2022 по сегодня?"
    python orchestrator.py --no-validator "Реальная ставка сейчас?"
    python orchestrator.py --sequential "Индекс нищеты март 2022?"
"""
...
# ── Задание 1: набор допустимых инструментов для валидации плана ──────────
...
def validate_plan(plan: Plan) -> list[str]:
    ...
    Проверяет:
    - все expected_tools существуют в VALID_TOOLS
    - depends_on ссылаются на реальные id
    - нет самозависимости
    - нет циклов в depends_on (DFS)
    """
    errors: list[str] = []
...
# ── Задание 2: топологические уровни ─────────────────────────────────────
