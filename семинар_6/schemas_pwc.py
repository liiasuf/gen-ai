...
- Исполнитель возвращает WorkerAnswer с сырым трейсом.
- Критик возвращает Verdict: ok/не ok, причина, действие (accept/replan/rework).
"""
...
class WorkerAnswer(BaseModel):
    ...
    raw_trace: list[dict[str, Any]] = Field(
        default_factory=list,
        description="Сырой trace из run_agent() для диагностики.",
    )
